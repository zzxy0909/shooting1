...
public class GUIEasyJoystickInspector : Editor{
	...
	public override void OnInspectorGUI(){
		...
		if (t.showProperties){
            ...
            if (t.drawOnlyWhenMove) GUI.backgroundColor = Color.green; else GUI.backgroundColor = Color.red;
            t.drawOnlyWhenMove = EditorGUILayout.Toggle("Draw only when move", t.drawOnlyWhenMove);
		...
		if (t.showPosition){
			...
			if (t.DynamicJoystick){
                ...
                t.areaWeight = EditorGUILayout.Vector2Field("Free area weight", t.areaWeight);
                t.followedArea = EditorGUILayout.Toggle("Followed area", t.followedArea);
				GUI.backgroundColor = Color.white;
			}
			else{	
				GUI.backgroundColor = Color.cyan;
