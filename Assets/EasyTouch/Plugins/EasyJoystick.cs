...
public class EasyJoystick : MonoBehaviour {
    ...
    public bool drawOnlyWhenMove = false;
    ...
    public Vector2 areaWeight = Vector2.one;

    public bool followedArea = false;
	...
	void OnGUI(){
		...
		if (enable && visible && (!drawOnlyWhenMove || moved)){
				...
				if (!dynamicJoystick){
					...
					if (!virtualJoystick){
						...
						switch (area){
							...
							// bottom left
							case DynamicArea.BottomLeft:
								if (gesture.position.y< Screen.height/2*areaWeight.y && gesture.position.x< Screen.width/2*areaWeight.x){
									virtualJoystick = true;
                                    touched = true;
                                }
							break;							
				...
				if (gesture.fingerIndex == joystickIndex){
					if (((gesture.position - center).sqrMagnitude < (zoneRadius *VirtualScreen.xRatio )*(zoneRadius *VirtualScreen.xRatio) && resetFingerExit) || !resetFingerExit) {
						...
						if ((joystickTouch/(zoneRadius-touchSizeCoef)).sqrMagnitude > 1){
                            if (followedArea)
                            {
                                float diffLength = joystickTouch.magnitude - (zoneRadius - touchSizeCoef);
                                Vector2 diffVector = joystickTouch.normalized * diffLength;
                                diffVector.y *= -1.0f;

                                joystickCenter += diffVector;
                            }
                        ...
                        if (deadZone * deadZone < joystickTouch.sqrMagnitude)
                            moved = true;
					}
					else{
						On_TouchUp( gesture);	
